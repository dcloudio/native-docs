...
1.在 info.plist 添加 `FacebookAppID` `FacebookClientToken`项，填写Facebook `appid`和`clientToken`，参考如下：
...
除苹果授权登录外，都需要实现如下方法
In addition to the Apple authorization login, the following methods need to be implemented
